...
const FOCUS_GROUP_SELECTORS = [
  ...
  ".final-cta__card",
  ".coordonnees__grid--hero",
  ".contact-form__head",
  ".acces__head, .acces__aside, .acces__actions, .acces__map",
];
...
export function initSite() {
  ...
  function initScrollRail() {
    ...
    if (!siteScroll || !rail) return;

    let ticking = false;

    const update = () => {
      ticking = false;
      const max = siteScroll.scrollHeight - siteScroll.clientHeight;
      const progress = max > 0 ? siteScroll.scrollTop / max : 0;
  ...
  function initStageDormancy() {
    ...
    if (!canvas) {
      stage.classList.add("is-dormant");
      return null;
    }

    if (!heroShell || !siteScroll || !("IntersectionObserver" in window)) {
      return null;
    }

    const observer = new IntersectionObserver(
      ([entry]) => {
        stage.classList.toggle("is-dormant", !entry.isIntersecting);
        siteScroll.dispatchEvent(new Event("scroll"));
      },
      { root: siteScroll, threshold: 0 }
    );
  ...
  function initHeroGlowFade() {
    ...
    const update = () => {
      ...
      if (!stage || stage.classList.contains("is-dormant")) {
  ...
  function hasFastWebGL() {
    ...
    return !/swiftshader|llvmpipe|software|basic render|paravirtual/i.test(driver);
  }
  ...
  function startWorker() {
    ...
    return {
      ...
      awake: (source, value) => worker.postMessage({ type: "awake", source, value }),
      dispose: () => {
  ...
  function bindScene(api) {
    ...
    visibility =
      heroShell && siteScroll && "IntersectionObserver" in window
        ? new IntersectionObserver(
            ([entry]) => {
              api.awake("view", entry.isIntersecting);
            },
            { root: siteScroll, threshold: 0 }
          )
        : null;
  ...
  function initMecanismeTimeline() {
    ...
    const update = () => {
      ...
      const scrollRect = siteScroll.getBoundingClientRect();
      const viewportTriggerY = scrollRect.top + scrollRect.height * readTriggerRatio();
      ...
      list.style.setProperty("--mecanisme-track-progress", progress.toFixed(4));
  ...
  function initViewportFocus() {
    ...
    const readFocusConfig = () => {
      ...
      return {
        centerRatio:
          parseFloat(root.getPropertyValue("--focus-center-ratio").trim()) || 0.38,
        lerp: parseFloat(root.getPropertyValue("--focus-lerp").trim()) || 0.12,
        dimLerp: parseFloat(root.getPropertyValue("--focus-dim-lerp").trim()) || 0.05,
        dimScaleSmooth:
          parseFloat(root.getPropertyValue("--focus-dim-scale-smooth").trim()) || 0.72,
      };
    };
    ...
    const tick = (now = performance.now()) => {
      ...
      groups.forEach((group) => {
        const targets = group.map((el) => targetStrength(el, viewportCenterY, reach));
        ...
        group.forEach((el, index) => {
          ...
          el.style.setProperty("--focus-dim-scale-value", nextDimScale.toFixed(4));
  ...
  function initSmoothScroll() {
    ...
    const readConfig = () => {
      ...
      return {
        ...
        velocityScale:
          parseFloat(root.getPropertyValue("--scroll-velocity-scale").trim()) || 3.8,
        decay: parseFloat(root.getPropertyValue("--scroll-decay").trim()) || 3.4,
      };
    };
  ...
  const stageDormancyObserver = initStageDormancy();
  ...
  const cleanup = () => {
    faqAccordionCleanup?.();
    smoothScrollCleanup?.();
    stageDormancyObserver?.disconnect();
    visibility?.disconnect();
    sceneApi?.dispose();
    siteCleanup = null;
  };
