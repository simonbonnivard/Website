...
const CONFIG = {
  ...
  logo: {
    ...
    additiveColor: 0xffcc55,
    additiveOpacity: 0.12,
  },
};
...
function buildLights() {
  ...
  const rimBack = new THREE.DirectionalLight(0xffb63c, 1.45);
  ...
  const accentFill = new THREE.DirectionalLight(0xffc247, 0.72);
  accentFill.position.set(4.8, 2.2, 2.4);
  scene.add(accentFill);

  const fill = new THREE.PointLight(0xffc94d, 7, 9.5, 2);
...
function buildCell() {
  ...
  const bodyMat = new THREE.MeshPhysicalMaterial({
    ...
    sheen: 0.12,
    sheenRoughness: 0.58,
    sheenColor: new THREE.Color(0xffe0a0),
    iridescence: 0.04,
    iridescenceIOR: 1.28,
...
function buildSparks(count) {
  ...
  const mat = new THREE.PointsMaterial({
    ...
    size: Q.sparkSize,
    map: tex,
    color: new THREE.Color().setRGB(1.12, 1.0, 0.68),
    transparent: true,
    opacity: 0,
    blending: THREE.AdditiveBlending,
    depthWrite: false,
